...
# def download_file(url, folder="file4", retries=3, delay=2):
#     try:
#         os.makedirs(folder, exist_ok=True)
#         filename = os.path.join(folder, url.split("/")[-1])
...
#             try:
#                 print(f"Downloading (Attempt {attempt}): {filename}")
...
#     except Exception as e:
#         print(f"Error setting up download for {url}: {e}")
...
#         "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf",
#         "https://images.pexels.com/photos/884788/pexels-photo-884788.jpeg?_gl=1*1pdn3y0*_ga*OTMxOTg5MTkuMTc1NzkyNjU5NQ..*_ga_8JE65Q40S6*czE3NTc5MjY1OTUkbzEkZzEkdDE3NTc5MjcyMzkkajU2JGwwJGgw.png/Sample2-png-image-500kb.png",    
#         "https://invalid-url-test.com/file.png"  # this will fail, shows retry
#     ]
...
# Metadata storage
metadata = []
metadata_file = "download_metadata.pkl"
lock = threading.Lock()  # for thread-safe writes to metadata


def save_metadata(entry):
    """Save metadata entry to pickle file (thread-safe)."""
    with lock:
...
def download_file(url, folder="downloads", retries=3, delay=2):
    try:
        os.makedirs(folder, exist_ok=True)
        filename = os.path.join(folder, url.split("/")[-1])
        status = "Failed"
        ...
        # Save metadata whether success or fail
        entry = {
            "filename": filename,
            "url": url,
            "status": status,
...
def main():
    urls = [
        "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf",
        "https://plus.unsplash.com/premium_photo-1666278379770-440439b08656?q=80&w=988&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D/Sample1.png",
        "https://images.unsplash.com/photo-1527529482837-4698179dc6ce?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D/Sample2.png"
    ]
